...
export function transformToCanonical(sheets: ParsedSheets): CanonicalDataset {
  ...
  return {
    ...
    // NEU: IoT, VoIP, Provisionen, OMO
    iotTariffs: (sheets.iot_tariffs ?? []).map(row => ({
      id: row.id as string,
      name: row.name as string,
      category: (row.category as string) ?? "standard",
      data_volume_mb: row.data_volume_mb as number,
      data_volume_text: row.data_volume_text as string,
      monthly_net: row.monthly_net as number,
      minTermMonths: (row.mintermmonths as number) ?? 24,
      overage_per_mb_net: row.overage_per_mb_net as number,
      provision_new: (row.provision_new as number) ?? 0,
      provision_renewal: row.provision_renewal as number,
      features: row.features as string,
      use_cases: row.use_cases as string,
      sort_order: row.sort_order as number,
      active: row.active as boolean ?? true,
    })),
    voipProducts: (sheets.voip_products ?? []).map(row => ({
      id: row.id as string,
      name: row.name as string,
      tier: (row.tier as string) ?? "standard",
      price_per_user_net: row.price_per_user_net as number,
      min_users: (row.min_users as number) ?? 1,
      max_users: row.max_users as number,
      minTermMonths: (row.mintermmonths as number) ?? 12,
      billing_cycle: (row.billing_cycle as "monthly" | "annual") ?? "monthly",
      included_minutes_de: row.included_minutes_de as number | "unlimited",
      included_minutes_intl: row.included_minutes_intl as number,
      video_conferencing: row.video_conferencing as boolean ?? false,
      team_messaging: row.team_messaging as boolean ?? false,
      sms_enabled: row.sms_enabled as boolean ?? false,
      hardware_options: row.hardware_options as string,
      features: row.features as string,
      provision_per_user: (row.provision_per_user as number) ?? 0,
      provision_setup: row.provision_setup as number,
      sort_order: row.sort_order as number,
      active: row.active as boolean ?? true,
    })),
    voipHardware: (sheets.voip_hardware ?? []).map(row => ({
      id: row.id as string,
      brand: row.brand as string,
      model: row.model as string,
      category: (row.category as "desk_phone" | "conference_phone" | "headset" | "accessory") ?? "desk_phone",
      ek_net: row.ek_net as number,
      uvp_net: row.uvp_net as number,
      compatible_tiers: row.compatible_tiers as string,
      features: row.features as string,
      image_url: row.image_url as string,
      sort_order: row.sort_order as number,
      active: row.active as boolean ?? true,
    })),
    provisions: (sheets.provisions ?? []).map(row => ({
      tariff_id: row.tariff_id as string,
      tariff_type: (row.tariff_type as "mobile" | "fixednet" | "iot" | "voip") ?? "mobile",
      provision_new_net: row.provision_new_net as number,
      provision_renewal_net: row.provision_renewal_net as number,
      provision_renewal_pct: row.provision_renewal_pct as number,
      fh_partner_modifier: row.fh_partner_modifier as number,
      push_modifier: row.push_modifier as number,
      notes: row.notes as string,
    })),
    omoMatrix: (sheets.omo_matrix ?? []).map(row => ({
      tariff_id: row.tariff_id as string,
      omo_0: (row.omo_0 as number) ?? 0,
      omo_5: row.omo_5 as number | null,
      omo_10: row.omo_10 as number | null,
      omo_15: row.omo_15 as number | null,
      omo_17_5: row.omo_17_5 as number | null,
      omo_20: row.omo_20 as number | null,
      omo_25: row.omo_25 as number | null,
      notes: row.notes as string,
    })),
  };
}
